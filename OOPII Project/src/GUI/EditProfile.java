...
public class EditProfile extends javax.swing.JFrame {
    ...
    private Date curDob;
    private BufferedImage curProfilePic;

    public EditProfile(Profile profile) throws SQLException, IOException {
        initComponents();
        editForm();
        fillData();
    ...
    private void initComponents() {
        ...
        editError.setText("signUpError");
        editError.setToolTipText("");
    ...
    private void editForm() {
        ...
        this.contentPanel.setBorder(BorderFactory.createTitledBorder(null, "Edit Profile", TitledBorder.CENTER,
        ...
        this.editError.setFont(new Font(Design.FONT_FAMILY, 1, Design.FONT_SIZE_SMALL));
    ...
    private void fillData() throws SQLException, IOException {
        Account account = getAccount(DataBaseConnection.SESSION);
                this.curFirstName = account.getFirstName();
                this.firstName.setText(this.curFirstName);
                this.curLastName = account.getLastName();
                this.lastName.setText(this.curLastName);
                this.curEmail = account.getEmail();
                this.email.setText(this.curEmail);

                this.curPassword = decrypt(account.getPassword());
                this.password.setText(this.curPassword);
                this.confirmPassword.setText(this.curPassword);

                if ("male".equals(account.getGender())) {
                    this.curGender = "male";
                    this.maleButton.setSelected(true);
                } else {
                    this.curGender = "female";
                    this.femaleButton.setSelected(true);
                }

                this.curDob = account.getDateOfBirth();
                this.dateOfBirth.setDate(this.curDob);

                File file = new File("images/" + account.getProfilePicture());
                this.curProfilePic = ImageIO.read(file);

                ImageIcon icon = new ImageIcon(file.getAbsolutePath());
                Image img = icon.getImage();
                int ratio = this.curProfilePic.getHeight() / this.profilePictureError.getHeight();
                Image imgScale = img.getScaledInstance(this.curProfilePic.getWidth() / ratio,
                        this.curProfilePic.getHeight() / ratio, Image.SCALE_SMOOTH);
                ImageIcon scaledIcon = new ImageIcon(imgScale);

                this.profilePictureError.setIcon(scaledIcon);
                this.profilePicture = this.curProfilePic;
    }
    ...
    public void updateAccount() throws ClassNotFoundException, SQLException, IOException {
        boolean canUpdate;
        int accountId = getAccountId(email.getText());
        if (accountId != -1) {
            if (accountId != DataBaseConnection.SESSION) {
                this.editError.setForeground(Color.RED);
                this.editError.setText("This email already has an account");
                canUpdate = false;
            } else {
                canUpdate = true;
            }
        ...
        if (canUpdate) {
            ...
            if (!this.firstName.getText().equals(this.curFirstName)) {
                updateFirstName(this.firstName.getText());
            }

            if (!this.lastName.getText().equals(this.curLastName)) {
                updateLastName(this.lastName.getText());
            }

            if (!this.email.getText().equals(this.curEmail)) {
                updateEmail(this.email.getText());
            }

            if (!this.password.getText().equals(this.curPassword)) {
                updatePassword(encrypt(this.password.getText()));
            }

            if (!"male".equals(this.curGender)) {
                if (this.maleButton.isSelected()) {
                    updateGender("male");
                }
            }

            if (!"female".equals(this.curGender)) {
                if (this.femaleButton.isSelected()) {
                    updateGender("female");
                }
            }

            Date newDob = new Date(dateOfBirth.getDate().getTime());
            if (!newDob.equals(this.curDob)) {
                updateDateOfBirth(newDob);
            }

            if (this.profilePicture != this.curProfilePic) {
                String pictureName = getProfilePicture(DataBaseConnection.SESSION);
                File file;
                if (!"user.jpg".equals(pictureName)) {
                ...
                updateProfilePicture(pictureName);
            }
            fillData();
